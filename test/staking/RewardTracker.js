...
describe("RewardTracker", function () {
  ...
  beforeEach(async () => {
    ...
    rewardDistributor = await deployContract("RewardDistributor", [esFxdx.address, rewardTracker.address])
    await rewardDistributor.updateLastDistributionTime()

    await rewardTracker.initialize([fxdx.address, esFxdx.address], rewardDistributor.address)
  })

  it("inits", async () => {
    expect(await rewardTracker.isInitialized()).eq(true)
    expect(await rewardTracker.isDepositToken(wallet.address)).eq(false)
    expect(await rewardTracker.isDepositToken(fxdx.address)).eq(true)
    expect(await rewardTracker.isDepositToken(esFxdx.address)).eq(true)
    expect(await rewardTracker.distributor()).eq(rewardDistributor.address)
    expect(await rewardTracker.distributor()).eq(rewardDistributor.address)
    expect(await rewardTracker.rewardToken()).eq(esFxdx.address)

    await expect(rewardTracker.initialize([fxdx.address, esFxdx.address], rewardDistributor.address))
  ...
  it("withdrawToken", async () => {
    ...
    await fxdx.mint(rewardTracker.address, 2000)
    await expect(rewardTracker.connect(user0).withdrawToken(fxdx.address, user1.address, 2000))
      .to.be.revertedWith("Governable: forbidden")

    await rewardTracker.setGov(user0.address)

    expect(await fxdx.balanceOf(user1.address)).eq(0)
    await rewardTracker.connect(user0).withdrawToken(fxdx.address, user1.address, 2000)
    expect(await fxdx.balanceOf(user1.address)).eq(2000)
  })

  it("stake, unstake, claim", async () => {
    await esFxdx.setMinter(wallet.address, true)
    await esFxdx.mint(rewardDistributor.address, expandDecimals(50000, 18))
    await rewardDistributor.setTokensPerInterval("20667989410000000") // 0.02066798941 esFxdx per second
    await fxdx.setMinter(wallet.address, true)
    await fxdx.mint(user0.address, expandDecimals(1000, 18))

    await rewardTracker.setInPrivateStakingMode(true)
    await expect(rewardTracker.connect(user0).stake(fxdx.address, expandDecimals(1000, 18)))
    ...
    await fxdx.connect(user0).approve(rewardTracker.address, expandDecimals(1000, 18))
    await rewardTracker.connect(user0).stake(fxdx.address, expandDecimals(1000, 18))
    expect(await rewardTracker.stakedAmounts(user0.address)).eq(expandDecimals(1000, 18))
    expect(await rewardTracker.depositBalances(user0.address, fxdx.address)).eq(expandDecimals(1000, 18))
    ...
    await esFxdx.connect(user1).approve(rewardTracker.address, expandDecimals(500, 18))
    await rewardTracker.connect(user1).stake(esFxdx.address, expandDecimals(500, 18))
    expect(await rewardTracker.stakedAmounts(user1.address)).eq(expandDecimals(500, 18))
    expect(await rewardTracker.stakedAmounts(user0.address)).eq(expandDecimals(1000, 18))
    expect(await rewardTracker.depositBalances(user0.address, fxdx.address)).eq(expandDecimals(1000, 18))
    expect(await rewardTracker.depositBalances(user0.address, esFxdx.address)).eq(0)
    expect(await rewardTracker.depositBalances(user1.address, fxdx.address)).eq(0)
    expect(await rewardTracker.depositBalances(user1.address, esFxdx.address)).eq(expandDecimals(500, 18))
    expect(await rewardTracker.totalDepositSupply(fxdx.address)).eq(expandDecimals(1000, 18))
    expect(await rewardTracker.totalDepositSupply(esFxdx.address)).eq(expandDecimals(500, 18))
    ...
    await expect(rewardTracker.connect(user0).unstake(esFxdx.address, expandDecimals(1001, 18)))
      .to.be.revertedWith("RewardTracker: _amount exceeds stakedAmount");

    await expect(rewardTracker.connect(user0).unstake(esFxdx.address, expandDecimals(1000, 18)))
      .to.be.revertedWith("RewardTracker: _amount exceeds depositBalance");

    await expect(rewardTracker.connect(user0).unstake(fxdx.address, expandDecimals(1001, 18)))
      .to.be.revertedWith("RewardTracker: _amount exceeds stakedAmount");

    expect(await fxdx.balanceOf(user0.address)).eq(0)
    await rewardTracker.connect(user0).unstake(fxdx.address, expandDecimals(1000, 18))
    expect(await fxdx.balanceOf(user0.address)).eq(expandDecimals(1000, 18))
    expect(await rewardTracker.totalDepositSupply(fxdx.address)).eq(0)
    expect(await rewardTracker.totalDepositSupply(esFxdx.address)).eq(expandDecimals(500, 18))
    ...
    await expect(rewardTracker.connect(user0).unstake(fxdx.address, 1))
      .to.be.revertedWith("RewardTracker: _amount exceeds stakedAmount");

    expect(await esFxdx.balanceOf(user0.address)).eq(0)
    await rewardTracker.connect(user0).claim(user2.address)
    expect(await esFxdx.balanceOf(user2.address)).gt(expandDecimals(1785 + 1190, 18))
    expect(await esFxdx.balanceOf(user2.address)).lt(expandDecimals(1786 + 1191, 18))
    ...
    await fxdx.mint(user1.address, expandDecimals(300, 18))
    await fxdx.connect(user1).approve(rewardTracker.address, expandDecimals(300, 18))
    await rewardTracker.connect(user1).stake(fxdx.address, expandDecimals(300, 18))
    expect(await rewardTracker.totalDepositSupply(fxdx.address)).eq(expandDecimals(300, 18))
    expect(await rewardTracker.totalDepositSupply(esFxdx.address)).eq(expandDecimals(500, 18))
    ...
    await expect(rewardTracker.connect(user1).unstake(fxdx.address, expandDecimals(301, 18)))
      .to.be.revertedWith("RewardTracker: _amount exceeds depositBalance");

    await expect(rewardTracker.connect(user1).unstake(esFxdx.address, expandDecimals(501, 18)))
      .to.be.revertedWith("RewardTracker: _amount exceeds depositBalance");
    ...
    expect(await esFxdx.balanceOf(user2.address)).eq(await rewardTracker.cumulativeRewards(user0.address))
    expect(await esFxdx.balanceOf(user3.address)).eq(await rewardTracker.cumulativeRewards(user1.address))

    expect(await fxdx.balanceOf(user1.address)).eq(0)
    expect(await esFxdx.balanceOf(user1.address)).eq(0)
    await rewardTracker.connect(user1).unstake(fxdx.address, expandDecimals(300, 18))
    expect(await fxdx.balanceOf(user1.address)).eq(expandDecimals(300, 18))
    expect(await esFxdx.balanceOf(user1.address)).eq(0)
    await rewardTracker.connect(user1).unstake(esFxdx.address, expandDecimals(500, 18))
    expect(await fxdx.balanceOf(user1.address)).eq(expandDecimals(300, 18))
    expect(await esFxdx.balanceOf(user1.address)).eq(expandDecimals(500, 18))
    expect(await rewardTracker.totalDepositSupply(fxdx.address)).eq(0)
    expect(await rewardTracker.totalDepositSupply(esFxdx.address)).eq(0)

    await rewardTracker.connect(user0).claim(user2.address)
    await rewardTracker.connect(user1).claim(user3.address)

    const distributed = expandDecimals(50000, 18).sub(await esFxdx.balanceOf(rewardDistributor.address))
  ...
  it("stakeForAccount, unstakeForAccount, claimForAccount", async () => {
    ...
    await rewardDistributor.setTokensPerInterval("20667989410000000") // 0.02066798941 esFxdx per second
    await fxdx.setMinter(wallet.address, true)
    await fxdx.mint(wallet.address, expandDecimals(1000, 18))

    await rewardTracker.setInPrivateStakingMode(true)
    await expect(rewardTracker.connect(user0).stake(fxdx.address, expandDecimals(1000, 18)))
      .to.be.revertedWith("RewardTracker: action not enabled")

    await expect(rewardTracker.connect(user2).stakeForAccount(wallet.address, user0.address, fxdx.address, expandDecimals(1000, 18)))
    ...
    await fxdx.connect(wallet).approve(rewardTracker.address, expandDecimals(1000, 18))

    await rewardTracker.connect(user2).stakeForAccount(wallet.address, user0.address, fxdx.address, expandDecimals(1000, 18))
    expect(await rewardTracker.stakedAmounts(user0.address)).eq(expandDecimals(1000, 18))
    expect(await rewardTracker.depositBalances(user0.address, fxdx.address)).eq(expandDecimals(1000, 18))
    ...
    await expect(rewardTracker.connect(user2).unstakeForAccount(user0.address, esFxdx.address, expandDecimals(1000, 18), user1.address))
      .to.be.revertedWith("RewardTracker: forbidden")
    ...
    await expect(rewardTracker.connect(user2).unstakeForAccount(user0.address, fxdx.address, expandDecimals(1001, 18), user1.address))
      .to.be.revertedWith("RewardTracker: _amount exceeds stakedAmount")
    ...
    await rewardTracker.connect(user2).unstakeForAccount(user0.address, fxdx.address, expandDecimals(100, 18), user1.address)
    ...
    expect(await rewardTracker.depositBalances(user0.address, fxdx.address)).eq(expandDecimals(900, 18))
