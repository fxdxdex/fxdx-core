...
async function main() {
  ...
  // const migrationTokens = ["GMT", "XGMT", "GMT_USDF", "XGMT_USDF"]

  const fxdxMigrator = await contractAt("FxdxMigrator", "0x0472F402EA8E301D7595545884Ad4C420E9865d6")
  const gmt = {
    name: "GMT",
  ...
  const tokens = [gmt, xgmt, gmtUsdf, xgmtUsdf]
  ...
  if (shouldApprove) {
    for (let i = 0; i < tokens.length; i++) {
      ...
      const migratedAmount = await fxdxMigrator.migratedAmounts(account, token.contract.address)
      ...
      await sendTxn(fxdxMigrator.setMaxMigrationAmount(account, token.contract.address, totalAmount), message)
    }
  }
}
