...
async function main() {
  ...
  console.log("NFT ID,Start (ETH),End (ETH),ETH Liquidity, FXDX Liquidity")
  for (let i = 0; i < nftIds.length; i++) {
    ...
    const start = tickToPrice(fxdx, weth, positionInfo.tickUpper).toSignificant(6)
    const end = tickToPrice(fxdx, weth, positionInfo.tickLower).toSignificant(6)
    const ethLiquidity = position.amount0.toSignificant(6)
    const fxdxLiquidity = position.amount1.toSignificant(6)

    console.log(`${nftId},${start},${end},${ethLiquidity},${fxdxLiquidity}`)
  }
}
