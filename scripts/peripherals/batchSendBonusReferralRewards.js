...
async function getArbValues() {
  ...
  return { batchSender, fxdx, data, gasLimit, totalFxdx, totalUsd }
}

async function getAvaxValues() {
  const batchSender = await contractAt("BatchSender", "0xF0f929162751DD723fBa5b86A9B3C88Dc1D4957b")
  const fxdx = await contractAt("Token", "0x62edc0692BD897D2295872a9FFCac5425011c661")
  const data = avaxData
...
async function main() {
  ...
  const { batchSender, fxdx, data, totalFxdx, totalUsd, gasLimit } = await getValues()
  ...
  if (shouldSendTxn) {
    ...
    await processBatch([affiliateAccounts, affiliateAmounts], batchSize, async (currentBatch) => {
      ...
      await sendTxn(batchSender.sendAndEmit(fxdx.address, accounts, amounts, affiliateRewardsTypeId, { gasLimit }), "batchSender.sendAndEmit(fxdx, affiliate rewards)")
    })
  }
}
