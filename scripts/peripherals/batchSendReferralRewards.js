...
async function getArbValues() {
  ...
  return { batchSender, esFxdx, nativeTokenPrice, data, gasLimit }
}

async function getAvaxValues() {
  const batchSender = await contractAt("BatchSender", "0xF0f929162751DD723fBa5b86A9B3C88Dc1D4957b")
  const esFxdx = await contractAt("Token", "0xFf1489227BbAAC61a9209A08929E4c2a526DdD17")
...
async function main() {
  ...
  const { batchSender, esFxdx, nativeTokenPrice, data } = await getValues()
  ...
  for (let i = 0; i < affiliatesData.length; i++) {
    const { account, rebateUsd, esfxdxRewardsUsd } = affiliatesData[i]
    ...
    if (esfxdxRewardsUsd) {
      const esFxdxAmount = bigNumberify(esfxdxRewardsUsd).mul(expandDecimals(1, 18)).div(expandDecimals(fxdxPrice, 30))
  ...
  if (shouldSendTxn) {
    ...
    await processBatch([esFxdxAccounts, esFxdxAmounts], batchSize, async (currentBatch) => {
      ...
      await sendTxn(batchSender.sendAndEmit(esFxdx.address, accounts, amounts, affiliateRewardsTypeId), "batchSender.sendAndEmit(nativeToken, esFxdx affiliate rewards)")
    })
  }
}
