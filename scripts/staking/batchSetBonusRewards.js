...
const { DISTRIBUTION_LIST } = require("../../data/esFxdxDistribution/distributionList1")
...
async function main() {
  ...
  for (let i = 0; i < distributionList.length; i++) {
    ...
    if (accounts.length === batchSize) {
      ...
      await sendTxn(timelock.batchSetBonusRewards(flpVester.address,  accounts, amounts), "timelock.batchSetBonusRewards")
