...
import "./interfaces/IFxdxIou.sol";
import "./interfaces/IAmmRouter.sol";
import "./interfaces/IFxdxMigrator.sol";

contract FxdxMigrator is ReentrancyGuard, IFxdxMigrator {
    ...
    modifier onlyAdmin() {
        require(msg.sender == admin, "FxdxMigrator: forbidden");
        _;
    }

    modifier onlySigner() {
        require(isSigner[msg.sender], "FxdxMigrator: forbidden");
        _;
    }
    ...
    function initialize(
        ...
        require(!isInitialized, "FxdxMigrator: already initialized");
        require(_whitelistedTokens.length == _iouTokens.length, "FxdxMigrator: invalid _iouTokens.length");
        require(_whitelistedTokens.length == _prices.length, "FxdxMigrator: invalid _prices.length");
        require(_whitelistedTokens.length == _caps.length, "FxdxMigrator: invalid _caps.length");
        require(_lpTokens.length == _lpTokenAs.length, "FxdxMigrator: invalid _lpTokenAs.length");
        require(_lpTokens.length == _lpTokenBs.length, "FxdxMigrator: invalid _lpTokenBs.length");

        isInitialized = true;

        ammRouter = _ammRouter;
        fxdxPrice = _fxdxPrice;
    ...
    function migrate(
        ...
        require(isMigrationActive, "FxdxMigrator: migration is no longer active");
        require(whitelistedTokens[_token], "FxdxMigrator: token not whitelisted");
        require(_tokenAmount > 0, "FxdxMigrator: invalid tokenAmount");

        if (hasMaxMigrationLimit) {
            migratedAmounts[msg.sender][_token] = migratedAmounts[msg.sender][_token].add(_tokenAmount);
            require(migratedAmounts[msg.sender][_token] <= maxMigrationAmounts[msg.sender][_token], "FxdxMigrator: maxMigrationAmount exceeded");
        }

        uint256 tokenPrice = getTokenPrice(_token);
        uint256 mintAmount = _tokenAmount.mul(tokenPrice).div(fxdxPrice);
        require(mintAmount > 0, "FxdxMigrator: invalid mintAmount");

        tokenAmounts[_token] = tokenAmounts[_token].add(_tokenAmount);
        require(tokenAmounts[_token] < caps[_token], "FxdxMigrator: token cap exceeded");
        ...
        if (lpTokens[_token]) {
            ...
            require(tokenA != address(0), "FxdxMigrator: invalid tokenA");
            require(tokenB != address(0), "FxdxMigrator: invalid tokenB");
        ...
        IFxdxIou(iouToken).mint(msg.sender, mintAmount);
    }
    ...
    function signApprove(address _token, address _spender, uint256 _amount, uint256 _nonce) external nonReentrant onlySigner {
        ...
        require(!signedActions[msg.sender][action], "FxdxMigrator: already signed");
    ...
    function getTokenPrice(address _token) public view returns (uint256) {
        ...
        require(price != 0, "FxdxMigrator: invalid token price");
        return price;
    }

    function getIouToken(address _token) public view returns (address) {
        address iouToken = iouTokens[_token];
        require(iouToken != address(0), "FxdxMigrator: invalid iou token");
        return iouToken;
    }
    ...
    function _validateAction(bytes32 _action) private view {
        require(pendingActions[_action], "FxdxMigrator: action not signalled");
    }
    ...
    function _validateAuthorization(bytes32 _action) private view {
        ...
        if (count == 0) {
            revert("FxdxMigrator: action not authorized");
        }
        require(count >= minAuthorizations, "FxdxMigrator: insufficient authorization");
    }

    function _clearAction(bytes32 _action, uint256 _nonce) private {
        require(pendingActions[_action], "FxdxMigrator: invalid _action");
