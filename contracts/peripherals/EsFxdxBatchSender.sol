...
contract EsFxdxBatchSender {
    using SafeMath for uint256;

    address public admin;
    address public esFxdx;

    constructor(address _esFxdx) public {
        admin = msg.sender;
        esFxdx = _esFxdx;
    }

    modifier onlyAdmin() {
        require(msg.sender == admin, "EsFxdxBatchSender: forbidden");
        _;
    }
    ...
    function send(
        ...
        for (uint256 i = 0; i < _accounts.length; i++) {
            IERC20(esFxdx).transferFrom(msg.sender, _accounts[i], _amounts[i]);
